...
// F16的朴素版本
__global__ void hgemm_naive_f16_kernel(const half* a,const half* b,half* c,int M,int N,int K){
                 ...
                 torch::Tensor &output_matrix, float alpha, float beta)
{
    ...
    TORCH_CHECK(matrix_a.device().is_cuda(), "Matrix A must be on CUDA device");
    TORCH_CHECK(matrix_b.device().is_cuda(), "Matrix B must be on CUDA device");
    TORCH_CHECK(matrix_a.dtype() == torch::kHalf, "Matrix A must be half");
    TORCH_CHECK(matrix_b.dtype() == torch::kHalf, "Matrix B must be half");
    TORCH_CHECK(matrix_a.dim() == 2, "Matrix A must be 2D");
    TORCH_CHECK(matrix_b.dim() == 2, "Matrix B must be 2D");
    ...
    TORCH_CHECK(matrix_b.size(0) == K, "Matrix dimensions must match: A is MxK, B must be KxN");
    TORCH_CHECK(output_matrix.device().is_cuda(), "Matrix C must be on CUDA device");
    TORCH_CHECK(output_matrix.dtype() == torch::kHalf, "Matrix C must be float32");
    TORCH_CHECK(output_matrix.size(0) == M && output_matrix.size(1) == N, "Matrix C must be MxN");
    ...
    constexpr uint block_size = 32;
    dim3 block_dim(block_size * block_size);
    dim3 grid_dim(ceil_div(M, block_size),
                  ceil_div(N, block_size));

    // Launch kernel

    hgemm_naive_f16_kernel<block_size><<<grid_dim, block_dim>>>(
        num_rows_a, num_cols_b, num_cols_a,
        alpha, d_matrix_a, d_matrix_b, beta, d_output_matrix);
}
                 ...
                 torch::Tensor &output_matrix, float alpha, float beta)
{
    ...
    TORCH_CHECK(matrix_a.device().is_cuda(), "Matrix A must be on CUDA device");
    TORCH_CHECK(matrix_b.device().is_cuda(), "Matrix B must be on CUDA device");
    TORCH_CHECK(matrix_a.dtype() == torch::kHalf, "Matrix A must be float32");
    TORCH_CHECK(matrix_b.dtype() == torch::kHalf, "Matrix B must be float32");
    TORCH_CHECK(matrix_a.dim() == 2, "Matrix A must be 2D");
    TORCH_CHECK(matrix_b.dim() == 2, "Matrix B must be 2D");
