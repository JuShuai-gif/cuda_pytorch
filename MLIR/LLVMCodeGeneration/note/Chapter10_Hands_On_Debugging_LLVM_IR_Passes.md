...
一份可交接的问题包应同时包含：原始输入、完整命令行、LLVM commit、CMake 配置、target
triple/CPU/features、环境变量、stderr、退出码，以及最后一个正确 IR 和第一个错误 IR。

推荐顺序：

1. **稳定复现**：固定线程数、随机种子、工作目录和动态库搜索路径。
2. **二分流水线**：使用 `-print-before/after`、`-stop-before/after` 找到第一个坏状态。
3. **立即验证**：`-verify-each` 让非法 IR 在制造它的 pass 后失败，而不是到 ISel 才崩溃。
4. **自动缩减**：interestingness test 必须检查特定错误特征和退出码，不能把超时、缺文件也当成“复现”。
5. **固化回归**：最小输入进入 lit/FileCheck；若是错编，再增加 baseline/candidate 语义测试。
6. **检查副作用**：修复正确性后继续检查编译时间、峰值内存、代码大小和运行性能。

对偶发问题，先证明它是否依赖未初始化内存、数据竞争或哈希/遍历顺序。Sanitizer、固定 seed 和
多次循环复现比盲目增加日志更有效。日志中不要输出用户模型权重、源码密钥或完整生产数据。
