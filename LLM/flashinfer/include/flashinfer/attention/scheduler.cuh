...
inline auto PrefillBinarySearchKVChunkSize(const bool enable_cuda_graph,
    ...
    const size_t batch_size = packed_qo_len_arr.size();
    ...
    while (low < high) {
        ...
        for (uint32_t i = 0; i < batch_size; ++i) {
            new_batch_size += ceil_div(packed_qo_len_arr[i], qo_chunk_size) * ceil_div(std::max(kv_len_arr[i], min_kv_len), mid);
        }
...
inline cudaError_t BatchDecodeWithPagedKVCacheWorkEstimationDispatched(
    ...
    DISPATCH_COMPUTE_CAP_DECODE_NUM_STAGES_SMEM(compute_capacity, NUM_STAGES_SMEM, {
        ...
        const uint32_t smem_size =
            2 * NUM_STAGES_SMEM * tile_size_per_bdx * bdy * bdz * HEAD_DIM * sizeof(DTypeKV) + std::max(tile_size_per_bdx * num_threads * sizeof(DTypeKV *), 2 * bdy * bdz * sizeof(float));
...
inline cudaError_t BatchDecodeWithPagedKVCacheWorkEstimationDispatchedMLA(
    ...
    DISPATCH_COMPUTE_CAP_DECODE_NUM_STAGES_SMEM(compute_capacity, NUM_STAGES_SMEM, {
        ...
        const uint32_t smem_size =
            NUM_STAGES_SMEM * bdy * bdz * (HEAD_DIM_CKV + HEAD_DIM_KPE) * sizeof(DTypeKV) + std::max(num_threads * sizeof(size_t) * 2, 2 * bdy * bdz * sizeof(float));
...
struct DecodePlanInfo {
    ...
    DecodePlanInfo() : padded_batch_size(0),
                       v_offset(0),
                       s_offset(0),
...
struct PrefillPlanInfo {
    ...
    PrefillPlanInfo() : padded_batch_size(0),
                        total_num_rows(0),
...
struct PrefillPlanSM90Info {
    ...
    PrefillPlanSM90Info() : qo_tile_indices_offset(0),
                            qo_indptr_offset(0),
                            kv_indptr_offset(0),
                            qo_len_offset(0),
                            kv_len_offset(0),
...
inline cudaError_t PrefillSM90Plan(
        ...
        for (auto &[i, qo_len, kv_len] : idx_qo_kv_len_vec) {
            ...
            for (int qo_tile_idx = num_qo_tiles - 1; qo_tile_idx >= 0; --qo_tile_idx) {
                ...
                int effective_kv_len =
                    causal ? packed_causal_kv_end(qo_len, kv_len, qo_tile_idx, cta_tile_q, num_qo_tiles, 1) : kv_len;
    ...
    if (enable_cuda_graph) {
        max_total_num_works = plan_info.same_schedule_for_all_heads ? max_num_works_per_head : max_num_works_per_head * num_qo_heads;
    } else {
            ...
            if (split_kv) {
                ...
                for (int offset_start = 0; offset_start < row_tile_size; offset_start += row_chunk_size) {
                    ...
                    merge_packed_offset_end[merge_cta_counter] =
                        qo_indptr_h[i] * num_heads + qo_tile_idx * cluster_tile_q + std::min(offset_start + row_chunk_size, current_q_tile_end);
