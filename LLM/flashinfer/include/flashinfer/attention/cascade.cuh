...
// 线程块内同步状态：将状态写入共享内存，然后从共享内存加载并合并所有线程的状态。
...
// warp内同步状态：将状态写入共享内存，然后从共享内存加载并合并所有线程的状态（使用__syncwarp()）。
template <uint32_t bdx, uint32_t bdy, uint32_t vec_size, typename DTypeIn>
...
// 线程块内求和：将向量写入共享内存，然后从共享内存加载并累加所有线程的向量。
template <uint32_t bdx, uint32_t bdy, uint32_t vec_size, typename DTypeIn>
...
// 注意力求和核函数：计算多个索引集的V向量的和（无logsumexp权重）。
template <uint32_t vec_size, typename DTypeIn, typename DTypeO>
...
// 合并多个索引集的自注意力状态核函数：基于logsumexp加权合并多个状态。
template <uint32_t vec_size, typename DTypeIn, typename DTypeO>
 ...
 */
// 合并多个索引集的自注意力状态核函数（针对大量索引集优化）：使用共享内存多级流水线。
 ...
 * \note s are logsumexp values with base 2.
 */
// 持久化变长合并状态核函数：支持可变长度索引集（每个位置的索引集数量可变），使用共享内存流水线。
template <uint32_t vec_size, uint32_t bdx, uint32_t bdy, uint32_t num_smem_stages, typename DTypeIn,
...
// 持久化变长注意力求和核函数：支持可变长度索引集，计算V向量的和（无logsumexp权重）。
template <uint32_t vec_size, uint32_t bdx, uint32_t bdy, uint32_t num_smem_stages, typename DTypeIn,
          typename DTypeO, typename IdType>
 ...
 * \note Both s_a and s_b are logsumexp values with base 2.
 */
// 合并两个索引集的自注意力状态（主机端函数）：调用MergeStateKernel核函数。
 ...
 * \note Both s and s_other are logsumexp values with base 2.
 */
// 原地合并自注意力状态（主机端函数）：调用MergeStateInPlaceKernel核函数，支持掩码。
 ...
 * \note s are logsumexp values with base 2.
 */
// 合并多个索引集的自注意力状态（主机端函数）：根据索引集数量选择不同的核函数实现。
template <typename DTypeIn, typename DTypeO>
...
// 注意力求和（主机端函数）：计算多个索引集的V向量的和（无logsumexp权重）。
template <typename DTypeIn, typename DTypeO>
...
// 变长合并状态（主机端函数）：支持可变长度索引集，调用PersistentVariableLengthMergeStatesKernel核函数。
...
// 变长注意力求和（主机端函数）：支持可变长度索引集，调用PersistentVariableLengthAttentionSumKernel核函数。
template <typename DTypeIn, typename DTypeO, typename IdType>
