...
# ============================================================
# Pool2DOptions — Pool2DOptions — TFLite 算子参数表。存储在 Operator.BuiltinOptions 字段中，通过 BuiltinOptionsType 枚举类型确定具体解析方式。TfliteConvertor 在解析算子时将其转换为 TinyEngine IR 参数。
# ============================================================
...
# ============================================================
# FlatBuffer 根节点解析入口
# 从内存中的 FlatBuffer 二进制数据中解析出 Pool2DOptions 表
# 调用方式: obj = Pool2DOptions.GetRootAsPool2DOptions(buf, offset)
#   - buf: bytes 类型，整个 FlatBuffer 二进制数据
#   - offset: 起始偏移量（通常为 0）
# TfliteConvertor.loadTFmodel() 通过 Pool2DOptions.GetRootAsPool2DOptions(buf, 0) 调用
# ============================================================
    @classmethod
...
# 字段不存在/未设置时返回默认值
    ...
    # Pool2DOptions
# 读取 FilterWidth 字段（int32（有符号32位整数））
...
# 字段不存在/未设置时返回默认值
    ...
    # Pool2DOptions
# 读取 FilterHeight 字段（int32（有符号32位整数））
