...
# ============================================================
# ConcatEmbeddingsOptions — ConcatEmbeddingsOptions — TFLite 算子参数表。存储在 Operator.BuiltinOptions 字段中，通过 BuiltinOptionsType 枚举类型确定具体解析方式。TfliteConvertor 在解析算子时将其转换为 TinyEngine IR 参数。
# ============================================================
...
# ============================================================
# FlatBuffer 根节点解析入口
# 从内存中的 FlatBuffer 二进制数据中解析出 ConcatEmbeddingsOptions 表
# 调用方式: obj = ConcatEmbeddingsOptions.GetRootAsConcatEmbeddingsOptions(buf, offset)
#   - buf: bytes 类型，整个 FlatBuffer 二进制数据
#   - offset: 起始偏移量（通常为 0）
# TfliteConvertor.loadTFmodel() 通过 ConcatEmbeddingsOptions.GetRootAsConcatEmbeddingsOptions(buf, 0) 调用
# ============================================================
    @classmethod
...
# ------------------------------------------------------------
# 初始化 ConcatEmbeddingsOptions 表对象
...
# 将 Table 访问器绑定到 buf 的 pos 位置
    ...
    # ConcatEmbeddingsOptions
# 读取 NumChannels 字段（int32（有符号32位整数））
...
# 字段不存在/未设置时返回默认值
    ...
    # ConcatEmbeddingsOptions
# 读取 NumColumnsPerChannel 向量字段（int32（有符号32位整数） 数组）
# vtable 偏移量: 6
...
# 读取该位置的 int32（有符号32位整数） 值
    ...
    # ConcatEmbeddingsOptions
# 以 Numpy ndarray 形式返回 NumColumnsPerChannel 向量的全部元素
# vtable 偏移量: 6
    ...
    # ConcatEmbeddingsOptions
# 返回 NumColumnsPerChannel 向量的长度（元素个数）
# vtable 偏移量: 6
...
# VectorLen(o) 读取向量头部的长度字段
    ...
    # ConcatEmbeddingsOptions
# 读取 NumColumnsPerChannelIsNone 向量字段（标量 数组）
# vtable 偏移量: 6
...
# 读取该位置的 标量 值
    ...
    # ConcatEmbeddingsOptions
# 读取 EmbeddingDimPerChannel 向量字段（int32（有符号32位整数） 数组）
# vtable 偏移量: 8
...
# 读取该位置的 int32（有符号32位整数） 值
    ...
    # ConcatEmbeddingsOptions
# 以 Numpy ndarray 形式返回 EmbeddingDimPerChannel 向量的全部元素
# vtable 偏移量: 8
    ...
    # ConcatEmbeddingsOptions
# 返回 EmbeddingDimPerChannel 向量的长度（元素个数）
# vtable 偏移量: 8
...
# VectorLen(o) 读取向量头部的长度字段
    ...
    # ConcatEmbeddingsOptions
# 读取 EmbeddingDimPerChannelIsNone 向量字段（标量 数组）
# vtable 偏移量: 8
