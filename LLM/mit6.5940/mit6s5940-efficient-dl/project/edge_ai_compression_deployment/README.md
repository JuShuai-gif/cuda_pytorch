...
这个最终项目应该承担“工业闭环验收”的角色：不是只演示 train/prune/quantize/export，而是要回答每个优化是否真的值得上线。后续重构要求见 ../../MISSING_AND_EXPANSION.md 的 P1-M8。

每次运行完整流水线都应尽量产出以下 artifact：

- baseline / pruned / quantized / distilled checkpoint。
- ONNX 或其他部署格式导出状态。
- benchmark JSON 原始数据。
- Markdown 报告，包含 accuracy delta、latency P50/P95/P99、throughput、model size、peak memory、output error 和 skipped reason。

工业验收时要特别注意：FLOPs 下降不等于 latency 下降；非结构化稀疏如果没有 sparse kernel 通常不会加速；INT8/PTQ 必须用代表性校准数据；TensorRT/OpenVINO/TFLite Micro 的收益必须在目标硬件上实测。
...
完整默认配置更接近课程项目，但训练和 benchmark 会较慢。开发或 CI 时先用 quick 模式确认流水线可运行：

```bash
python main.py --mode full_pipeline --quick
```

`--quick` 会自动切换到 `tinycnn`、少量 synthetic samples、1 epoch 训练/微调/蒸馏，以及短 benchmark。它不是精度结论，只用于验证工程链路没有断。
