...
| 时间紧、INT8、模型常规 | PTQ |
| 精度敏感、低比特、数据可训练 | QAT |
| LLM W4A16 | AWQ/GPTQ 类 weight-only PTQ |
| 激活 outlier 明显 | SmoothQuant 或混合精度 |
| 回归/action head | 保留最后几层 FP16/FP32 或单独校准 |

### 12.2 QAT 的关键点

QAT 不是“训练 int8 模型”，而是在训练中用 fake quant 模拟量化误差，让模型适应最终 integer runtime。必须保证：

- fake quant 的 round/clamp 与目标 runtime 一致。
- Conv+BN 已融合或训练中正确处理 BN。
- observer freeze 和 batchnorm freeze 时机正确。
- 训练后一定用目标 runtime 做 parity check。

### 12.3 对应代码实验

```bash
python src/lecture-06/main.py
python src/model_compression/benchmark_compression.py --quant-bits 8 --runs 5 --warmup 2 --train-steps 1
```

报告中至少比较：baseline、PTQ、dynamic quant、可选 ONNX Runtime。

### 12.4 本讲验收问题

1. PTQ 为什么需要 calibration？
2. QAT 中 fake quant 的 forward/backward 分别做什么？
3. 什么时候应该用 mixed precision？
4. 为什么 LayerNorm/Softmax 通常不直接 INT8？
5. 如何设计 PyTorch 与 TensorRT 的数值一致性测试？

## 13. Python 代码补充：FakeQuantize 的 STE 写法

QAT 的核心是 forward 模拟量化误差，backward 用 straight-through estimator 让梯度能传回浮点权重。
...
class FakeQuantSTE(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output):
        # STE: pretend quantization is identity for gradient
...
工业解读：QAT 训练出的模型还必须用目标 runtime 做一致性测试。FakeQuant 的 round/clamp 规则如果和 TensorRT/ONNX Runtime 不一致，部署后仍会掉精度。
