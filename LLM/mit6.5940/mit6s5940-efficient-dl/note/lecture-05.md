...
## 12. 学习闭环补充：量化不是 dtype cast

### 12.1 本讲工业核心

量化的工程对象不是单个 tensor，而是完整 graph：observer 收集范围，scale/zero-point 决定整数表达，graph rewrite 插入 Q/DQ 或替换 quantized op，runtime kernel 负责整数计算和 requantization。

| 概念 | 工业意义 |
|---|---|
| scale / zero point | 浮点和整数之间的 contract |
| observer | 校准数据如何决定范围 |
| per-channel | 解决不同输出通道 range 差异 |
| symmetric | 硬件实现更简单，常用于权重 |
| asymmetric | 表达非零中心分布，常用于激活 |
| saturation | 超出量化范围的信息被截断 |

### 12.2 校准数据要求

校准数据不是越多越好，而是要覆盖目标分布：

- 分类：覆盖常见类和长尾类。
- 检测：覆盖不同尺度、小目标、遮挡、暗光。
- LLM：覆盖不同 prompt 长度、语言、代码、数学、长上下文。
- VLA：覆盖不同场景、动作幅度、失败边界样本。

### 12.3 对应代码实验

```bash
python src/lecture-05/main.py
python src/model_compression/benchmark_compression.py --quant-bits 8 --runs 5 --warmup 2 --train-steps 1
```

补充阅读：`note/quantization_industry_playbook.md`。

### 12.4 本讲验收问题

1. 对称量化和非对称量化分别适合什么？
2. 为什么 per-channel 权重量化通常比 per-tensor 好？
3. 什么是 saturation rate？为什么上线前要看？
4. 为什么 INT8 模型可能比 FP32 慢？
5. 如何判断量化误差来自权重还是激活？

## 13. Python 代码补充：对称 INT8 量化与 SQNR

下面代码展示最小可用的对称量化、反量化和 SQNR 计算。它是理解 PTQ、QAT 和 LLM weight-only quantization 的基础。
...
w[0] = 8.0  # outlier
w_hat, scale, q = symmetric_quant_dequant(w, bits=8)
print("scale", float(scale))
print("SQNR(dB)", float(sqnr_db(w, w_hat)))
```

工业解读：一个 outlier 会撑大 scale，让绝大多数正常值落在很少的 int8 bins 中。这就是 activation outlier 会破坏 Transformer 量化的原因。
