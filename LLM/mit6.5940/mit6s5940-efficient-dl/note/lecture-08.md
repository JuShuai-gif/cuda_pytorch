...
## 12. 学习闭环补充：硬件感知 NAS 与 Once-for-All

### 12.1 工业核心

硬件感知 NAS 的关键是 latency lookup table 或真实测量闭环。同一个 architecture 在不同硬件上的排序可能完全不同：CPU 喜欢 cache-friendly，GPU 喜欢大矩阵，NPU 受 op support 和 layout 限制，MCU 受 SRAM 限制。

### 12.2 Once-for-All 思路

OFA 训练一个 supernet，再从里面抽取不同 subnet：

```text
train supernet -> progressive shrinking -> sample subnet -> evaluate latency/accuracy -> choose Pareto candidates
```
...
- 不同硬件 cost function 下的 best model。

### 12.4 本讲验收问题

1. 为什么同一个模型在 CPU 和 GPU 上 latency 排名不同？
2. Latency lookup table 如何构建？
3. OFA 为什么适合多设备部署？
4. 目标函数里如何平衡 accuracy 和 latency？
5. 搜索出的模型为什么还必须导出验证？

## 13. Python 代码补充：Pareto Frontier 提取

硬件感知 NAS 的输出不是一个模型，而是一组 Pareto 最优模型。

```python
def pareto_frontier(points):
    # point: {"acc": higher better, "lat": lower better, "arch": ...}
    frontier = []
    for p in points:
        dominated = False
        for q in points:
            better_or_equal = q["acc"] >= p["acc"] and q["lat"] <= p["lat"]
            strictly_better = q["acc"] > p["acc"] or q["lat"] < p["lat"]
            if better_or_equal and strictly_better:
                dominated = True
                break
        if not dominated:
            frontier.append(p)
    return sorted(frontier, key=lambda x: x["lat"])

points = [
    {"acc": 0.75, "lat": 2.0, "arch": "A"},
    {"acc": 0.77, "lat": 2.8, "arch": "B"},
    {"acc": 0.74, "lat": 1.6, "arch": "C"},
    {"acc": 0.76, "lat": 3.5, "arch": "D"},
]
print(pareto_frontier(points))
```

工业解读：上线时通常会保留多档 Pareto 模型，而不是只选最高精度模型。
