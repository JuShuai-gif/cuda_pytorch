...
## 12. 学习闭环补充：指标、Roofline 与真实 Profiling

### 12.1 本讲的工业核心

Lecture 02 的核心是建立 profiling 语言。工业优化不是先问“用什么算法”，而是先判断瓶颈：compute-bound、memory-bound、launch-bound、IO-bound 还是 data-pipeline-bound。

常用判断：

| 现象 | 可能瓶颈 | 优先动作 |
|---|---|---|
| FLOPs 高、GPU 利用率高 | compute-bound | FP16/INT8、Tensor Core、kernel fusion |
| FLOPs 低但 latency 高 | memory/launch-bound | fusion、layout、减少中间 tensor |
| batch 增大吞吐提升明显 | launch overhead 或 memory latency | dynamic batching |
| P99 远高于 P50 | 系统抖动/内存分配/调度 | 预分配、绑核、隔离、监控 |
| ONNX 比 PyTorch 慢 | op fallback/layout transform | 检查 provider 和 op kernel |
...
```text
model, device, runtime, input shape, batch size, dtype, warmup, repeat,
mean, P50, P95, P99, throughput, peak memory, dependency version
```
...
```bash
python src/lecture-02/main.py
python src/model_compression/benchmark_compression.py --runs 5 --warmup 2 --train-steps 1
```

补充阅读：

- `note/benchmarking_and_profiling.md`
- `src/model_compression/metrics.py`

### 12.4 本讲验收问题

1. 什么是 arithmetic intensity？它如何决定 Roofline 上限？
2. 为什么 depthwise conv FLOPs 很低但有时不快？
3. 为什么端侧交互看 P99，而数据中心看 throughput？
4. 如何发现 ONNX Runtime 某个 op fallback 到 CPU？
5. 模型大小、参数量、activation memory、workspace memory 有什么区别？

## 13. Python 代码补充：Roofline 输入指标计算

这段代码演示如何估算一个卷积层的 MACs、访存量和 arithmetic intensity。它不是精确 profiler，但能帮助你判断优化方向。

```python
def conv2d_macs(n, cin, cout, h, w, kh, kw, groups=1):
    return n * h * w * cout * (cin // groups) * kh * kw

def conv2d_bytes(n, cin, cout, h, w, kh, kw, dtype_bytes=4):
    input_bytes = n * cin * h * w * dtype_bytes
    weight_bytes = cout * cin * kh * kw * dtype_bytes
    output_bytes = n * cout * h * w * dtype_bytes
    return input_bytes + weight_bytes + output_bytes

n, cin, cout, h, w, kh, kw = 1, 64, 128, 56, 56, 3, 3
macs = conv2d_macs(n, cin, cout, h, w, kh, kw)
bytes_moved = conv2d_bytes(n, cin, cout, h, w, kh, kw)
arithmetic_intensity = macs / bytes_moved
print(f"MACs: {macs / 1e6:.1f} M")
print(f"Bytes: {bytes_moved / 1e6:.2f} MB")
print(f"Arithmetic intensity: {arithmetic_intensity:.2f} MAC/byte")
```
