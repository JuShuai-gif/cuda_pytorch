...
这个仓库的目标不是只保存课程笔记，而是把 MIT 6.S5940 的核心内容变成一套可运行、可验收、能迁移到工业部署的学习工程。当前项目仍在补齐中，后续修改以 [MISSING_AND_EXPANSION.md](MISSING_AND_EXPANSION.md) 为总控清单。

学习时不要只看 Markdown。每个阶段都应该完成四件事：

1. 读对应 `note/lecture-XX.md`，理解课程概念。
2. 跑对应 `src/lecture-XX/main.py` 或 `src/model_compression/benchmark_compression.py`，看到真实指标。
3. 完成对应 `labs/lab-XX`，把算法自己实现一遍。
4. 用 `project/edge_ai_compression_deployment` 做端到端压缩、导出和 benchmark。

掌握标准也不是“看完所有文件”，而是能独立回答并验证：模型变小了吗、真的变快了吗、精度掉了多少、目标硬件能不能跑、部署工具链哪里会失败。
...
- [MISSING_AND_EXPANSION.md](MISSING_AND_EXPANSION.md)：后续补齐路线和 Codex prompt。
- [note/model_compression_industry.md](note/model_compression_industry.md)：剪枝、量化、蒸馏、部署的工业实践。
- [project/edge_ai_compression_deployment/README.md](project/edge_ai_compression_deployment/README.md)：最终项目如何验收。

### 工业实战 Playbook

| 主题 | 文档 |
|---|---|
| 剪枝 | [note/pruning_industry_playbook.md](note/pruning_industry_playbook.md) |
| 量化 | [note/quantization_industry_playbook.md](note/quantization_industry_playbook.md) |
| Benchmark/Profiling | [note/benchmarking_and_profiling.md](note/benchmarking_and_profiling.md) |
| NAS | [note/nas_hardware_aware_playbook.md](note/nas_hardware_aware_playbook.md) |
| TinyML | [note/tinyml_deployment_playbook.md](note/tinyml_deployment_playbook.md) |
| LLM 推理 | [note/llm_inference_playbook.md](note/llm_inference_playbook.md) |
| ViT/Diffusion/VLA | [note/vision_diffusion_vla_playbook.md](note/vision_diffusion_vla_playbook.md) |
| 分布式与端侧训练 | [note/distributed_ondevice_playbook.md](note/distributed_ondevice_playbook.md) |
