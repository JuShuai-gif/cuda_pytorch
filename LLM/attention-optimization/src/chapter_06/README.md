...
This chapter contains the runnable artifacts for `FlashAttention V3 Hopper Simulation`. The implementation is intentionally educational: it favors explicit data movement, small reproducible examples, and clear metrics over maximum performance. Production caveats are documented in `notes/chapter_06.md`.

## Files

- `flash_attention_v3_sim.cu` - main implementation or reading guide.
...
cmake --build . --target flash_attention_v3_sim -j
```
...
```bash
./chapters/flash_attention_v3_sim
```

If the target is CUDA-based, profile it with:

```bash
ncu --set full ./chapters/flash_attention_v3_sim
nsys profile -o chapter_06_timeline ./chapters/flash_attention_v3_sim
```
