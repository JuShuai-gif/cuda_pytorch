...
│       └── diagrams/           ←  架构图
│
├── benchmarks/                 ← 生产级 Benchmark suite (PPL/MMLU/HumanEval/GSM8K/MATH)
├── cuda/                       ← 生产级 Triton kernels (FlashAttention/PagedAttention/fused ops/quantization)
├── diagrams/                   ← 9 张系统架构 Mermaid 图
├── distributed/                ← 生产级分布式训练 (TP/PP/FSDP/SP/EP/plan)
├── inference/                  ← 生产级推理引擎 (ContinuousBatching/SpeculativeDecoding/PrefixCache)
├── tokenizer/                  ← 生产级 BPE tokenizer (训练/编解码/HF兼容)
└── transformer/                ← Transformer 变体 (Llama3/Mistral/DeepSeek-V3 MLA/MoE)
```
