...
// 额外列bank冲突解决优化：在共享内存数组中添加额外列以避免bank冲突
// 模板参数：
// - BM: 块的行大小（Block行维度）
// - BN: 块的列大小（Block列维度）
...
// 优化目标：通过添加额外列（padding）改变数据布局，避免共享内存bank冲突
// 背景：当矩阵宽度是bank数量（32）的倍数时，同一warp中的线程可能访问同一bank，导致bank冲突
// 解决方案：添加额外列（如5列），使实际宽度不是32的倍数
...
__global__ void sgemmResolveBankExtraCol(int M, int N, int K, float alpha,
  ...
  // 关键优化：为Bs添加额外列以避免bank冲突
  // 假设BN=128，如果BN是32的倍数（128/32=4），则可能发生bank冲突
  // 添加extraCols=5列后，实际宽度为133，不是32的倍数，可以避免bank冲突
  const int extraCols = 5;  // 额外列数（padding）
  __shared__ float Bs[BK * (BN + extraCols)];  // 存储B矩阵的BK×(BN+extraCols)子块
  ...
  for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
    ...
    // 关键优化：加载B矩阵数据到带有额外列的共享内存
    // 使用简单线性索引：innerRowB * (BN + extraCols) + innerColB * 4 + i
    // 由于有额外列，同一warp中的线程访问不同bank，避免bank冲突
    ...
    for (uint dotIdx = 0; dotIdx < BK; ++dotIdx) {
      ...
      for (uint i = 0; i < TN; ++i) {
        // 关键优化：访问带有额外列的Bs数组
        // Bs[dotIdx * (BN + extraCols) + threadCol * TN + i]
        // 由于有额外列，同一warp中的线程访问不同bank，避免bank冲突
