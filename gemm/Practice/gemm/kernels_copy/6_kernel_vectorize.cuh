...
// 向量化优化：使用float4向量类型进行内存访问，提高内存带宽利用率
// 模板参数：
// - BM: 块的行大小（Block行维度）
// - BN: 块的列大小（Block列维度）
...
// 优化目标：使用128位向量加载/存储（float4），将内存带宽利用率提高4倍
...
__global__ void sgemmVectorize(int M, int N, int K, float alpha, float *A,
  ...
  {
    // 向量化加载A矩阵数据到共享内存
    // 关键优化：使用float4一次性加载4个连续的单精度浮点数（128位）
    // 示例：假设innerColA=0，则加载A[innerRowA*K + 0*4]开始的4个连续元素
    float4 tmp =
        reinterpret_cast<float4 *>(&A[innerRowA * K + innerColA * 4])[0];
    
    // 将向量化的4个元素存储到共享内存的不同位置
    // 注意：这里使用了转置存储（列主序），As的索引是(innerColA*4+i)*BM + innerRowA
    // 这可能是为了更好的内存访问模式或避免bank冲突
    As[(innerColA * 4 + 0) * BM + innerRowA] = tmp.x;  // 存储第一个元素
    As[(innerColA * 4 + 1) * BM + innerRowA] = tmp.y;  // 存储第二个元素
    As[(innerColA * 4 + 2) * BM + innerRowA] = tmp.z;  // 存储第三个元素
    As[(innerColA * 4 + 3) * BM + innerRowA] = tmp.w;  // 存储第四个元素
    ...
    for (uint dotIdx = 0; dotIdx < BK; ++dotIdx) {
      // 将数据从共享内存块加载到寄存器中
      for (uint i = 0; i < TM; ++i) {
        // 加载A的TM个元素：注意索引是dotIdx*BM + threadRow*TM + i
        // 这表示As是列主序存储：第dotIdx列，第(threadRow*TM+i)行
        regM[i] = As[dotIdx * BM + threadRow * TM + i];
      }
      for (uint i = 0; i < TN; ++i) {
        // 加载B的TN个元素：第dotIdx行，第(threadCol*TN+i)列
  ...
  // 将结果写回全局内存（输出矩阵C）
  // 关键优化：使用向量化存储（float4），一次性写入4个连续结果
