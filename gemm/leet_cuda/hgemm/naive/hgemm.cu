...
// 朴素HGEMM的PyTorch绑定函数：每个线程计算C矩阵中的一个元素
void hgemm_naive_f16(torch::Tensor a, torch::Tensor b, torch::Tensor c) {
  ...
  // 获取矩阵维度
  const int M = a.size(0);  // A矩阵的行数，C矩阵的行数
  const int K = a.size(1);  // A矩阵的列数，B矩阵的行数
  const int N = b.size(1);  // B矩阵的列数，C矩阵的列数
  ...
  // 线程块和网格配置
  constexpr int BM = 32;  // 每个线程块在M方向处理的大小
  constexpr int BN = 32;  // 每个线程块在N方向处理的大小
  
  dim3 block(BN, BM);  // 线程块维度：BN x BM个线程
  dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM);  // 网格维度

  // 调用CUDA内核
  hgemm_naive_f16_kernel<<<grid, block>>>(
    reinterpret_cast<half*>(a.data_ptr()),  // 将PyTorch张量指针转换为half指针
...
// 使用K维度分片和共享内存的HGEMM PyTorch绑定函数
void hgemm_sliced_k_f16(torch::Tensor a, torch::Tensor b, torch::Tensor c) {
  ...
  CHECK_TORCH_TENSOR_SHAPE(b, K, N)
  CHECK_TORCH_TENSOR_SHAPE(c, M, N)
  constexpr int BM = 32;  // 每个线程块处理C矩阵块的行数
  constexpr int BN = 32;  // 每个线程块处理C矩阵块的列数
  constexpr int BK = 32;  // K维度分块大小
...
// 线程分片8x8 + half2向量化的HGEMM PyTorch绑定函数
// 每个线程计算8x8个元素，使用half2向量化加载/存储
