...
// ============================================================================
// 文件: matmul_3.cuh
// 功能: 优化的WGMMA矩阵乘法，支持更大的tile大小和更复杂的流水线
// 特点:
//   - 扩展的WGMMA使用，支持更大的输出tile（256x128）
//   - 改进的双缓冲策略，支持更多阶段的流水线
//   - 使用更复杂的共享内存布局和bank冲突避免
//   - 支持异步屏障（async barrier）进行更细粒度的同步
//   - 优化了寄存器使用和指令调度
// 改进点（相比matmul_2）:
//   - 更大的block tile：256x128（vs 128x128）
//   - 更多的warp group：8个warp group（vs 4个）
//   - 更深的流水线：支持4阶段双缓冲
//   - 更好的负载均衡：更细粒度的线程分配
// 性能目标:
//   - 更高的计算密度（每个block处理更多数据）
//   - 更好的内存带宽利用率
//   - 更低的同步开销
// 适用场景:
//   - 超大矩阵乘法（M,N > 4096）
