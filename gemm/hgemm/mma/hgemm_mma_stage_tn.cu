...
static __device__ __forceinline__ int swizzle_permuted_B_j(int i, int j) {
    ...
    // | row 15 | (8, 0) |
    // -------------------
    return swizzle_permuted_j<kMmaAtomK>(i, j);
}
...
#pragma unroll
    for (int k = 0; k < (K_STAGE - 1); k++) { // 0, 1
        ...
        uint32_t load_smem_a_ptr =
            (smem_a_base_ptr + (k * s_a_stage_offset + load_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(load_smem_a_m, load_smem_a_k)) * sizeof(half));
        CP_ASYNC_CG(load_smem_a_ptr, &A[load_gmem_a_addr], 16); // MMA_K 0
        uint32_t load_smem_a_mma_k_ptr =
            (smem_a_base_ptr + s_a_mma_k_store_offset * sizeof(half) + (k * s_a_stage_offset + load_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(load_smem_a_m, load_smem_a_k)) * sizeof(half));
        CP_ASYNC_CG(load_smem_a_mma_k_ptr, &A[load_gmem_a_addr + 16],
                    16); // MMA_K 1

        uint32_t load_smem_b_ptr =
            (smem_b_base_ptr + (k * s_b_stage_offset + load_smem_b_n * (BK + B_PAD) + load_smem_b_k) * sizeof(half));
        CP_ASYNC_CG(load_smem_b_ptr, &B[load_gmem_b_addr], 16);

        uint32_t load_smem_b_mma_k_ptr =
            (smem_b_base_ptr + s_b_mma_k_store_offset * sizeof(half) + (k * s_b_stage_offset + load_smem_b_n * (BK + B_PAD) + load_smem_b_k) * sizeof(half));
...
#pragma unroll
        for (int i = 0; i < WARP_TILE_M; ++i) {
            ...
            uint32_t lane_smem_a_ptr = (smem_a_base_ptr + (0 * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half));
...
#pragma unroll
        for (int j = 0; j < WARP_TILE_N; ++j) {
            ...
            uint32_t lane_smem_b_ptr =
                (smem_b_base_ptr + (0 * s_b_stage_offset + lane_smem_b_n * (BK + B_PAD) + lane_smem_b_k) * sizeof(half));
...
#pragma unroll
    for (int k = (K_STAGE - 1); k < NUM_K_TILES; ++k) {
        ...
        uint32_t load_smem_a_ptr =
            (smem_a_base_ptr + (smem_sel_next * s_a_stage_offset + load_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(load_smem_a_m, load_smem_a_k)) * sizeof(half));
        CP_ASYNC_CG(load_smem_a_ptr, &A[load_gmem_a_addr], 16); // MMA_K 0
        uint32_t load_smem_a_mma_k_ptr =
            (smem_a_base_ptr + s_a_mma_k_store_offset * sizeof(half) + (smem_sel_next * s_a_stage_offset + load_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(load_smem_a_m, load_smem_a_k)) * sizeof(half));
        CP_ASYNC_CG(load_smem_a_mma_k_ptr, &A[load_gmem_a_addr + 16],
                    16); // MMA_K 1

        uint32_t load_smem_b_ptr =
            (smem_b_base_ptr + (smem_sel_next * s_b_stage_offset + load_smem_b_n * (BK + B_PAD) + load_smem_b_k) * sizeof(half));
        CP_ASYNC_CG(load_smem_b_ptr, &B[load_gmem_b_addr], 16);

        uint32_t load_smem_b_mma_k_ptr =
            (smem_b_base_ptr + s_b_mma_k_store_offset * sizeof(half) + (smem_sel_next * s_b_stage_offset + load_smem_b_n * (BK + B_PAD) + load_smem_b_k) * sizeof(half));
        CP_ASYNC_CG(load_smem_b_mma_k_ptr, &B[load_gmem_b_addr + 16], 16);
        CP_ASYNC_COMMIT_GROUP();
...
#pragma unroll
        for (int i = 0; i < WARP_TILE_M; ++i) {
            ...
            uint32_t lane_smem_a_ptr =
                (smem_a_base_ptr + s_a_mma_k_store_offset * sizeof(half) + (smem_sel * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half));
            LDMATRIX_X4(RA[reg_store_idx][i][0], RA[reg_store_idx][i][1],
                        RA[reg_store_idx][i][2], RA[reg_store_idx][i][3],
                        lane_smem_a_ptr);
        }
...
#pragma unroll
        for (int j = 0; j < WARP_TILE_N; ++j) {
            ...
            int lane_smem_b_k = ((lane_id / 8) % 2) * 8;     // 0,8
            uint32_t lane_smem_b_ptr =
                (smem_b_base_ptr + s_b_mma_k_store_offset * sizeof(half) + (smem_sel * s_b_stage_offset + lane_smem_b_n * (BK + B_PAD) + lane_smem_b_k) * sizeof(half));
...
#pragma unroll
        for (int i = 0; i < WARP_TILE_M; ++i) {
            ...
            int lane_smem_a_k = (lane_id / 16) * 8;           // 0,8
            uint32_t lane_smem_a_ptr =
                (smem_a_base_ptr + (smem_sel_reg * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half));
            LDMATRIX_X4(RA[reg_store_idx][i][0], RA[reg_store_idx][i][1],
                        RA[reg_store_idx][i][2], RA[reg_store_idx][i][3],
                        lane_smem_a_ptr);
        }
...
#pragma unroll
        for (int j = 0; j < WARP_TILE_N; ++j) {
            ...
            int lane_smem_b_k = ((lane_id / 8) % 2) * 8;     // 0,8
            uint32_t lane_smem_b_ptr =
                (smem_b_base_ptr + (smem_sel_reg * s_b_stage_offset + lane_smem_b_n * (BK + B_PAD) + lane_smem_b_k) * sizeof(half));
            LDMATRIX_X2(RB[reg_store_idx][j][0], RB[reg_store_idx][j][1],
                        lane_smem_b_ptr);
        }
    }
...
#pragma unroll
            for (int i = 0; i < WARP_TILE_M; ++i) {
                ...
                int lane_smem_a_k = (lane_id / 16) * 8;           // 0,8
                uint32_t lane_smem_a_ptr =
                    (smem_a_base_ptr + s_a_mma_k_store_offset * sizeof(half) + (stage_sel * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half));
                LDMATRIX_X4(RA[reg_store_idx][i][0], RA[reg_store_idx][i][1],
                            RA[reg_store_idx][i][2], RA[reg_store_idx][i][3],
                            lane_smem_a_ptr);
            }
...
#pragma unroll
            for (int j = 0; j < WARP_TILE_N; ++j) {
                ...
                int lane_smem_b_k = ((lane_id / 8) % 2) * 8;     // 0,8
                uint32_t lane_smem_b_ptr =
                    (smem_b_base_ptr + s_b_mma_k_store_offset * sizeof(half) + (stage_sel * s_b_stage_offset + lane_smem_b_n * (BK + B_PAD) + lane_smem_b_k) * sizeof(half));
                // may use .x4.trans to load 4 matrix for reg double buffers at once?
                LDMATRIX_X2(RB[reg_store_idx][j][0], RB[reg_store_idx][j][1],
                            lane_smem_b_ptr);
            }
...
#pragma unroll
            for (int i = 0; i < WARP_TILE_M; ++i) {
                ...
                int lane_smem_a_k = (lane_id / 16) * 8;           // 0,8
                uint32_t lane_smem_a_ptr =
                    (smem_a_base_ptr + (stage_sel_reg * s_a_stage_offset + lane_smem_a_m * (BK + A_PAD) + swizzle_permuted_A_j<MMA_K>(lane_smem_a_m, lane_smem_a_k)) * sizeof(half));
                LDMATRIX_X4(RA[reg_store_idx][i][0], RA[reg_store_idx][i][1],
                            RA[reg_store_idx][i][2], RA[reg_store_idx][i][3],
                            lane_smem_a_ptr);
            }
...
#pragma unroll
            for (int j = 0; j < WARP_TILE_N; ++j) {
                ...
                int lane_smem_b_k = ((lane_id / 8) % 2) * 8;     // 0,8
                uint32_t lane_smem_b_ptr =
                    (smem_b_base_ptr + (stage_sel_reg * s_b_stage_offset + lane_smem_b_n * (BK + B_PAD) + lane_smem_b_k) * sizeof(half));
                // may use .x4.trans to load 4 matrix for reg double buffers at once?
                LDMATRIX_X2(RB[reg_store_idx][j][0], RB[reg_store_idx][j][1],
                            lane_smem_b_ptr);
            }
...
#define LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X2_KERNEL(stages,                                                       \
    ...
    {                                                                                                                                \
        const int smem_max_size =                                                                                                    \
            ((stages) * BM * (BK + A_PAD) * WARP_TILE_K * sizeof(half) + (stages) * BN * (BK + B_PAD) * WARP_TILE_K * sizeof(half)); \
